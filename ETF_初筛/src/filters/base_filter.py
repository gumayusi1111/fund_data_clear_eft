...
class BaseFilter(ABC):
    ...
    def get_summary_stats(self, results: Dict[str, FilterResult]) -> Dict[str, Any]:
        ...
        stats = {
            "筛选器名称": self.name,
            "总ETF数": len(results),
            "通过数": len(passed_results),
            "未通过数": len(failed_results),
            "通过率": len(passed_results) / len(results) * 100,
            "平均得分": sum(r.score for r in results.values()) / len(results),
            "通过ETF得分": {
                "平均": sum(r.score for r in passed_results) / len(passed_results) if passed_results else 0,
    ...
    def calculate_trend_metrics(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            # 计算累计收益率
            总收益率 = (收盘价.iloc[-1] / 收盘价.iloc[0] - 1) * 100
            
            # 计算最大回撤
            累计净值 = (1 + df['涨幅%'] / 100).cumprod()
            高点 = 累计净值.cummax()
            回撤 = (累计净值 / 高点 - 1) * 100
            最大回撤 = 回撤.min()
            
            # 计算夏普比率（简化版，假设无风险利率为3%）
            年化收益率 = 总收益率 * (252 / len(df))  # 假设252个交易日
            年化波动率 = df['涨幅%'].std() * (252 ** 0.5)
            夏普比率 = (年化收益率 - 3) / 年化波动率 if 年化波动率 > 0 else 0
    ...
    def calculate_liquidity_metrics(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            # 成交量稳定性（变异系数）
            成交量稳定性 = df['成交量(手数)'].std() / df['成交量(手数)'].mean() if df['成交量(手数)'].mean() > 0 else float('inf')
            
            # 零成交量天数
            零成交量天数 = (df['成交量(手数)'] == 0).sum()
            零成交量比例 = 零成交量天数 / len(df) * 100
