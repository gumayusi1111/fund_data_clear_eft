...
class OutputManager:
    ...
    def save_simple_results(self, processing_results: Dict[str, Any], 复权类型: str) -> bool:
        """
        简化保存：保存两个不同门槛的筛选结果
    ...
    def _save_threshold_results(self, processing_results: Dict[str, Any], threshold_name: str):
        ...
        候选ETF列表 = processing_results.get("最终结果", {}).get("候选ETF列表", [])
        ...
        if 通过ETF列表:
            ...
            with open(passed_file, 'w', encoding='utf-8') as f:
                f.write(f"# 通过筛选ETF - {threshold_name} - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - 共{len(通过ETF列表)}个\n")
                for etf_code in 通过ETF列表:
                    # 移除.SZ/.SH后缀，只保留6位代码
                    clean_code = etf_code.split('.')[0] if '.' in etf_code else etf_code
                    f.write(f"{clean_code}\n")
            
            self.logger.info(f"✅ 保存{threshold_name}通过筛选ETF: {passed_file} ({len(通过ETF列表)}个)")
        ...
        if 候选ETF列表:
            ...
            with open(candidate_file, 'w', encoding='utf-8') as f:
                f.write(f"# 候选ETF - {threshold_name} - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - 共{len(候选ETF列表)}个\n")
                for etf_code in 候选ETF列表:
                    # 移除.SZ/.SH后缀，只保留6位代码
                    clean_code = etf_code.split('.')[0] if '.' in etf_code else etf_code
                    f.write(f"{clean_code}\n")
            
            self.logger.info(f"✅ 保存{threshold_name}候选ETF: {candidate_file} ({len(候选ETF列表)}个)")
        ...
        try:
            ...
            if 通过ETF列表:
                success_count += self._save_etf_data_to_directory(
                    通过ETF列表, etf_data, 复权类型, "通过筛选"
                )
            
            # 保存候选ETF数据（可选）
            if 候选ETF列表:
                success_count += self._save_etf_data_to_directory(
                    候选ETF列表, etf_data, 复权类型, "候选ETF", save_to_main=False
                )
            
            # 保存筛选结果报告
            self._save_processing_report(processing_results, 复权类型)
            
            self.logger.info(f"✅ 成功保存 {success_count} 个ETF数据到 {复权类型}")
            return True
    ...
    def save_all_processed_data(self, processing_results: Dict[str, Any],
        ...
        try:
            ...
            # 为每种复权类型保存数据
            for 复权类型 in 复权类型列表:
                self.logger.info(f"📁 保存 {复权类型} 数据...")
                
                # 加载通过ETF的该复权类型数据
                etf_data = source_data_loader.load_multiple_etfs(通过ETF列表, 复权类型)
                
                if etf_data:
                    # 保存到对应复权目录
                    self._save_etf_data_to_directory(
                        通过ETF列表, etf_data, 复权类型, "筛选结果"
                    )
                else:
                    self.logger.warning(f"⚠️ {复权类型} 数据加载失败")
    ...
    def _save_etf_data_to_directory(self, etf_codes: List[str],
        ...
        Returns:
            成功保存的文件数
        """
        output_dir = self.output_base / 复权类型
        output_dir.mkdir(parents=True, exist_ok=True)
        
        success_count = 0
        保留天数 = self.config.get_输出设置().get("保留天数", 252)
        ...
        self.logger.info(f"📊 {description}: 保存 {success_count}/{len(etf_codes)} 个ETF到 {复权类型}")
    ...
    def _save_processing_report(self, processing_results: Dict[str, Any], 
        ...
        try:
            ...
            report_file = report_dir / f"筛选报告_{复权类型}_{timestamp}.json"
    ...
    def _save_unified_report(self, processing_results: Dict[str, Any]):
        ...
        try:
            ...
            summary_data = {
                ...
                "通过ETF列表": processing_results.get("通过ETF", []),
                "复权数据可用性": {
                    复权类型: True for 复权类型 in self.config.get_复权类型()
                }
            }
    ...
    def _get_filter_config_summary(self) -> Dict[str, Any]:
        """获取筛选器配置摘要"""
        try:
            筛选条件 = self.config.get_筛选条件()
            return {
                "筛选器类型": ["质量筛选", "成交量筛选", "趋势筛选"],
                "主要条件": {
                    "最小交易天数": 筛选条件.get("基础条件", {}).get("最小交易天数"),
                    "最小平均成交量": 筛选条件.get("基础条件", {}).get("最小平均成交量_手"),
                    "最大回撤限制": 筛选条件.get("趋势条件", {}).get("最大回撤限制"),
                    "最小夏普比率": 筛选条件.get("趋势条件", {}).get("最小夏普比率")
                }
            }
        except Exception:
    ...
    def get_output_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 检查各复权目录
            for 复权类型 in self.config.get_复权类型():
                复权目录 = self.output_base / 复权类型
                if 复权目录.exists():
                    csv_files = list(复权目录.glob("*.csv"))
                    summary["复权目录状态"][复权类型] = {
                        "存在": True,
                        "ETF文件数": len(csv_files),
                        "最新更新": self._get_directory_latest_mtime(复权目录)
                    }
                else:
                    summary["复权目录状态"][复权类型] = {
                        "存在": False,
