...
def merge_and_save_etf_data(all_data: Dict[str, Dict[str, List]], mode: str = 'incremental'):
    ...
    for adj_type, category in category_map.items():
        ...
        for etf_code, rows in all_data[adj_type].items():
            ...
            etf_file = os.path.join(category_dir, f"{etf_code}.csv")
            ...
            if before_count > after_count:
                print(f"🧹 {etf_code}: 新数据去重 {before_count} → {after_count} 条记录")
            ...
            if mode == 'incremental' and os.path.exists(etf_file):
                ...
                try:
                    ...
                    print(f"🔄 {etf_code}: 合并后 {len(combined_df)} 条记录（去重完成）")
                    
                except Exception as e:
                    print(f"⚠️ 读取现有文件失败 {etf_code}.csv: {e}，使用新数据")
                    combined_df = new_df
            else:
                # 重建模式或文件不存在：直接使用新数据
                combined_df = new_df
                ...
                if before_count > after_count:
                    print(f"🧹 {etf_code}: 重建模式去重 {before_count} → {after_count} 条记录")
